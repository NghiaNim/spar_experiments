...
| mode                | pos rate | L0 ROC | L0 PR | best ROC (L)   | best PR (L)    | PR-lift over baseline | L0→L_best PR gain |
| ---                 | ---:     | ---:   | ---:  | ---            | ---            | ---:                  | ---:              |
...
| category              | pos rate | L0 ROC | L0 PR | best ROC (L)  | best PR (L)   | PR-lift over baseline | L0→L_best PR gain |
| ---                   | ---:     | ---:   | ---:  | ---           | ---           | ---:                  | ---:              |
...
| tactic         | pos rate | L0 ROC | L0 PR | best ROC (L)    | best PR (L)   | PR-lift over baseline | L0→L_best PR gain |
| ---            | ---:     | ---:   | ---:  | ---             | ---           | ---:                  | ---:              |
...
| deception ↔ sandbag         | (sandbag probe_weights.pt missing; pending probe-sandbag-v2 re-run with current code) |
| manipulation ↔ sandbag      | (pending re-run) |

For a hidden dimension of H ≈ 2048, two random unit vectors have expected cosine similarity ~N(0, 1/√H) ≈ 0 ± 0.022. The observed deception ↔ manipulation similarity of **+0.020 is at the random-direction noise floor**. Across the full (layer × offset) grid, the **maximum cell is +0.095 at L12, k=9** (4.3× the random-vector SD — small in absolute terms but statistically distinguishable from chance). Mean cosine across the grid is ≈+0.04. The slight positive bias at upper-mid layers is plausibly explained by both probes partially reading the shared hack-persona register signature there; the headline cell at L9, k=0 is closer to true noise floor.

**The mid-stack band is therefore home to three *co-located linear decoders*** rather than a shared "misalignment direction." This is consistent with — but does not establish — a shared mid-stack computational substrate; an orthogonal alternative explanation is that the L7–L13 residual stream has sufficient effective rank to host many independent linear features, of which the three misalignment-commitment features happen to be three. **One pairwise comparison (deception ↔ manipulation) is not enough to claim a 3-way orthogonality structure**; we report it as a single pair with the sandbag pair pending. If sandbag comes in at cos ≈ 0.05 with both other behaviors, the orthogonality story holds; if sandbag comes in at cos ≈ 0.25 with deception (plausible — both target binary-truth claims), the §4.1b conclusion *changes*. Both outcomes belong in the paper.
...
We confirmed that linear probing is the right tool by training a 2-layer MLP (hidden=256, GELU) on the **best-ROC-AUC layer per behavior** (sandbag L9, decep L9, manip L12) at k=0 and k=5, with the same data, split, and rebalancing as the linear sweep. Across six paired comparisons, the MLP scored within **|ΔROC-AUC| ≤ 0.012** of the linear probe in every cell, and was slightly *worse* on five of six (consistent with mild overfitting on the ~10k-token training sets). The misalignment representation is linearly accessible at the chosen layer.

A small consistency note: for sandbag, the best-PR-AUC layer at k=0 (L7) differs from the best-ROC-AUC layer (L9). We ran the MLP at L9 — the ROC-AUC headline — which is L9 across all three behaviors and so makes the comparison uniform; the trade-off is that the MLP check does not cover sandbag's PR-AUC headline cell at L7. The L9 vs L7 directions for sandbag are not the same (cos ≤ 0.46 across layers; §4.1b), so a separate L7 MLP run is queued as a follow-up. We expect the same result (linear matches MLP) — the linear-suffices conclusion is unlikely to flip between adjacent layers — but we should run it to close the loop. Full writeup: `writeups/mlp_sanity.md`.
...
6. **Probe-direction cosine similarity across behaviors** — **PARTIALLY DONE.** Deception ↔ manipulation cosine at L9, k=0 = +0.020 (random-direction noise floor); max across the grid is +0.095 at L12, k=9 (4× noise SD). Sandbag missing (weights not yet saved). One pairwise comparison is not enough to claim a 3-way orthogonality structure — sandbag probe re-run is queued. Within-experiment rotation rate is matched across behaviors (the actual cross-experiment regularity). Reported in §4.1b. (`writeups/probe_similarity.md`).
