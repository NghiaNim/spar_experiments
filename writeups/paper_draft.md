...
5. **MLP-vs-linear sanity check** — **DONE.** Linear matches MLP within |Δ| ≤ 0.012 ROC-AUC across seven (behavior × layer × offset) cells (including a follow-up sandbag L7 run for the PR-AUC headline); linear is the right tool. Reported in §4.1c. (`writeups/mlp_sanity.md`).
