...
Deception variant. Structurally identical to the manipulation /
sandbag model stage; only the seed-job import path and the per-record
extras carried into the corpus differ.
"""
...
from deception_experiment.seed_prompts import get_seed_jobs
...
def _batched_generate(
    ...
    """Left-padded batched sampling. Forwards deception-specific
    extras (``false_claim``, ``claim_category``, ``ground_truth``)
    onto each corpus record verbatim.
    """
    records: list[dict] = []
