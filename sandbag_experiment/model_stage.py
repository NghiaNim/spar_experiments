...
Sandbag variant. Structurally identical to ``reward_hack_experiment``'s
model stage:

  - Seed jobs carry ``sandbag_mode``, ``difficulty``, and
    ``ground_truth`` extras (see ``sandbag_experiment.seed_prompts``);
    the loop forwards them onto each corpus record so the labeler can
    use ``ground_truth`` and downstream QC can break results down by
    ``sandbag_mode`` / ``difficulty``.
  - Two ``prompt_kind`` values: ``"hack"`` (sandbag persona) and
    ``"honest"``.

Both kinds always have a system prompt — matched length — to remove the
...
from sandbag_experiment.seed_prompts import get_seed_jobs
...
def generate_and_extract(
    ...
    seed_jobs = get_seed_jobs(task=task, seed=seed)
...
def _batched_generate(
    ...
    Each job dict carries ``system_prompt``, ``user_prompt``, ``kind``,
    ``prompt_id`` plus sandbag-specific extras (``sandbag_mode``,
    ``difficulty``, ``ground_truth``) which are forwarded onto each
