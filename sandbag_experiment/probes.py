...
Sandbag variant. Identical machinery to ``reward_hack_experiment.probes``;
the only differences are the default positive-class kind name
(``"hack"`` here means sandbag-persona completions, mirroring the
reward-hack convention) and the plot titles.

Two sweeps are produced by default:

  - ``all/``             — every completion. NOTE: hack and honest use
                           different system prompts, so a probe here
                           can partially learn "is this a sandbag-
                           persona completion?" rather than token-level
                           lookahead. Use with caution.
  - ``sandbag_prompts/`` — only completions whose seed used the
                           sandbag (hack) system prompt. All samples
                           share the same hack system prompt, so the
                           class-presence confound is removed; this is
                           the cleaner read for token-level lookahead.

The keyword ``positive_kind`` defaults to ``"hack"`` (set on
``run_full_sweep``); the subdirectory is ``{positive_kind}_prompts``,
which is overridden in the entrypoint to ``"sandbag_prompts"``.
"""
...
def run_full_sweep(
    ...
    """Produce two sweeps (full corpus, sandbag-prompt subset).

    Output layout::

        {out_dir}/{run_name}/all/                results.json + *.png
        {out_dir}/{run_name}/{positive_subset_name}/  results.json + *.png

    ``positive_subset_name`` defaults to ``sandbag_prompts`` so the
    output reads as the sandbag-only sweep, even though the underlying
    ``prompt_kind`` value is ``"hack"``.
    """
    if run_name is None:
...
def _plot_heatmap(grid: np.ndarray, layers, offsets, path: Path, title: str) -> None:
    ...
    plt.title(f"Sandbag probe — {title} across layers × offsets")
    plt.tight_layout()
...
def _plot_lines(
    ...
    plt.title(f"Sandbag probe — {ylabel} vs offset (per layer)")
