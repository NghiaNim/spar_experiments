...
Manipulation variant. Structurally identical to
``sandbag_experiment.model_stage``; the only differences are the seed-job
import path and the per-record ``tactic`` extra (vs ``sandbag_mode`` /
``difficulty``).

Both ``hack`` and ``honest`` kinds always have a system prompt.
"""
...
from manipulation_experiment.seed_prompts import get_seed_jobs
...
def _batched_generate(
    ...
    """Left-padded batched sampling. Forwards manipulation-specific
    extras (``tactic``) onto each corpus record verbatim.
    """
    records: list[dict] = []
