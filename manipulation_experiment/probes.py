...
Manipulation variant. Identical machinery to ``sandbag_experiment.probes``;
the only differences are the default positive-class kind name
(``"hack"`` here means manipulator-persona completions) and the plot
titles.
...
Two sweeps are produced by default:
  ...
  - ``all/``                 — every completion. Hack and honest use
                                ...
                                manipulator completion?" rather than
                                token-level lookahead. Use with caution.
  - ``manipulation_prompts/`` — only completions whose seed used the
                                manipulator (hack) system prompt. All
                                samples share the same hack system
                                prompt, so the class-presence confound
                                is removed.
"""
...
def run_full_sweep(
    ...
    positive_subset_name: str = "manipulation_prompts",
) -> str:
    if run_name is None:
...
def _plot_heatmap(grid: np.ndarray, layers, offsets, path: Path, title: str) -> None:
    ...
    plt.title(f"Manipulation probe — {title} across layers × offsets")
    plt.tight_layout()
...
def _plot_aggregate_heatmap(grid, layers, prefix_lengths, path, title):
    ...
    plt.ylabel("transformer layer  (0 = embeddings)")
    plt.title(f"Manipulation probe — {title} (per-completion)")
    plt.tight_layout()
    plt.savefig(path, dpi=140)
    plt.close()
    print(f"saved plot -> {path}")
...
def _plot_lines(
    ...
    plt.title(f"Manipulation probe — {ylabel} vs offset (per layer)")
