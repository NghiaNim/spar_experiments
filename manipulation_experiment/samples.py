"""Build a human-readable ``samples.jsonl`` for manipulation QC.

One JSON object per line with everything you need to sanity-check the
labeled corpus by eye or with ``jq``.

Manipulation-specific extra: ``tactic`` (one of ``urgency`` /
``scarcity`` / ``flattery`` / ``social_proof`` / ``guilt`` for hack
...
Useful recipes::
    ...
    # break down positive rate by manipulation tactic
    jq -r '[.tactic, .prompt_kind, (.frac_pos|tostring)] | @tsv' \\
        data/manipulation/manipulation/samples.jsonl
"""
