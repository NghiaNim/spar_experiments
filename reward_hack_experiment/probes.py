...
Two sweeps are produced by default (reward-hack pipeline):
  - ``all/``            — every completion. NOTE: because the hack and
                          honest classes use different system prompts, a
                          probe here can partially learn "is this a hack-
                          persona completion?" rather than token-level
                          lookahead. Use with caution.
  - ``hack_prompts/``   — only completions whose seed used the hack
                          system prompt. All samples share the same hack
                          system prompt, so the class-presence confound
                          is removed; this is the cleaner read.

The positive-class kind name is configurable via ``positive_kind``
(default ``"hack"``); the sibling ``probe_experiment`` package uses
``"harm"``.
"""
...
def sweep_layers_and_offsets(
    ...
    prompt_filter: str | None = None,   # None | "hack" | "honest"
...
def run_full_sweep(
    ...
    """Produce two sweeps (full corpus, positive-prompt subset), keyed by run_name.

    Output layout (with default ``positive_kind="hack"``)::

        {out_dir}/{run_name}/all/           results.json + *.png
        {out_dir}/{run_name}/hack_prompts/  results.json + *.png
    ...
    subsets = [("all", None), (f"{positive_kind}_prompts", positive_kind)]
...
def _plot_heatmap(grid: np.ndarray, layers, offsets, path: Path, title: str) -> None:
    ...
    plt.title(f"Reward-hack probe — {title} across layers × offsets")
    plt.tight_layout()
...
def _plot_lines(
    ...
    plt.title(f"Reward-hack probe — {ylabel} vs offset (per layer)")
