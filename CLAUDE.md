...
v2 results are in for all three (see `writeups/sandbag_v2.md`, `writeups/manipulation_v2.md`, `writeups/deception_v2.md`). The v2 relabel **improved lift for sandbag and deception** (transition framing was the right one) but **decreased lift for manipulation** (the manipulation register is a sustained ongoing signal, not a fleeting commitment — collapsing labels to the prefix threw away the informative bits). Cite **sandbag v2** and **deception v2** within-hack numbers as the headlines for those probes; cite **manipulation v1** for manipulation. The strongest probe sits in the **L7–L13 mid-stack band** for all three core behaviors *regardless* of label variant (sandbag drifts L7→L5→L12 across offsets; deception is L7 in v1 and L9 in v2; manipulation is L11–L13 in both variants) — so the layer *band* is consistent but only **sandbag's** best layer actually migrated v1→v2 (L15→mid-stack). Manipulation and deception were already mid-stack under v1. Per-layer differences inside the band are 0.014–0.079 ROC-AUC, comparable to plausible seed variance; multi-seed reruns are the highest-priority follow-up. Amongst the three transition-labeled core behaviors, deception has the longest lookahead horizon (lift +0.10 at k=10) and sandbag the shortest (+0.02). Note: sycophancy `hack_prompts/` has ROC-AUC 0.82 at k=10, longer than any of the three core behaviors — so "longest lookahead" claims must be scoped to the three v2 core behaviors only. See `writeups/critique/paper_draft_critique_v2_details.md` for the full numerical-fact-check.
